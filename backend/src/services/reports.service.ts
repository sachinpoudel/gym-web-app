...
export const reportsService = {
  async getOverview() {
    ...
    const [
      ...
      prisma.member.count({ where: { status: MemberStatus.FROZEN } }),
      ...
      prisma.payment.aggregate({
        _sum: { amount: true },
        where: { status: PaymentStatus.PAID, paidAt: { gte: yearStart } }
      }),
      prisma.payment.aggregate({
        _sum: { amount: true },
        where: {
      ...
      prisma.attendance.count({ where: { checkIn: { gte: weekStart } } }),
      prisma.attendance.count({ where: { checkIn: { gte: monthStart } } }),
      prisma.gymClass.count(),
      prisma.gymClass.count({ where: { day: weekday } }),
      ...
      prisma.$queryRaw<Array<{ period: Date; total: number }>>`
  ...
  async getRevenue(period: "monthly" | "yearly") {
    if (period === "yearly") {
      const rows = await prisma.$queryRaw<Array<{ period: Date; total: number }>>`
  ...
  async getAttendance(startDate?: Date, endDate?: Date) {
